...
export const createList = async (name, isPublic) => {
  ...
  try {
    const response = await fetch('https://movie-server-nu.vercel.app/api/movielists', {
      method: 'POST',
      headers: {
...
export const addMovieToList = async (listId, movie, accessToken) => {
  const response = await fetch(`https://movie-server-nu.vercel.app/api/movielists/${listId}`, {
      method: 'PUT',
      headers: {
          'Content-Type': 'application/json',
